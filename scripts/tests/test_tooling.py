...
class GenerationTests(unittest.TestCase):
    ...
    def test_runtime_rejects_godot_before_4_3(self) -> None:
        with mock.patch.object(
            verify_project, "_run", return_value=(0, "4.2.2.stable\n")
        ):
            errors, _checks, evidence = verify_project._runtime_checks(
                Path("/tmp/example"), Path("/tmp/godot"), load_fixture()
            )
        self.assertIn("Godot 4.3 or newer", "\n".join(errors))
        self.assertEqual("4.2.2.stable", evidence["godot_version"])
